...
class Result {
    ...
    public static int beautifulTriplets(int d, List<Integer> arr) {
        int count = 0;
        // Use a HashSet for efficient O(1) average time complexity for lookups.
        Set<Integer> numSet = new HashSet<>(arr);

        // Iterate through each number in the original array.
        for (int num : arr) {
            // Check if the set contains the other two numbers that would form a beautiful triplet.
